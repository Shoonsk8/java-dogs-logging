    ...
    public void sendMessage()
    {
        for (Dog e : DogsinitialApplication.ourDogList.dogList)
        {
            ...
            if (rand <= 5)
            {
                ...
                rt.convertAndSend(DogsinitialApplication.Companion.getQUEUE_NAME_HIGH(), message);
            } else
            {
                logger.info("Sending message LOW");
                rt.convertAndSend(DogsinitialApplication.Companion.getQUEUE_NAME_LOW(), message);
            }
        }
    }
}
